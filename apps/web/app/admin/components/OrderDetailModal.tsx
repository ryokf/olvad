    ...
    return (
        ...
        >
            ...
            >
                {/* Header */}
                <div
                    className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 bg-gradient-to-br from-[#1a1a2e] to-[#16213e] border-b border-white/10"
                >
                    <div>
                        <p className="text-xs font-medium text-white/50">Detail Pesanan</p>
                        <h2 className="text-xl font-bold text-white">#{order.id}</h2>
                    </div>
                    <button
                        id="modal-close-btn"
                        onClick={onClose}
                        className="w-9 h-9 rounded-xl flex items-center justify-center text-white transition-all hover:opacity-70 bg-white/10"
                    >
                        ✕
                    </button>
                </div>
                ...
                <div className="p-6 space-y-5">
                    {/* Status Update */}
                    <div className="rounded-2xl p-5 space-y-4 bg-[#f8f9fb] border border-gray-200">
                        <h3 className="font-bold text-sm text-gray-700">⚡ Update Status</h3>
                        ...
                        <button
                            ...
                            className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${
                                hasChanges
                                    ? "bg-gradient-to-br from-primary-400 to-primary-500 text-white cursor-pointer"
                    ...
                    <div className="rounded-2xl p-5 border border-gray-200">
                        <h3 className="font-bold text-sm mb-4 text-gray-700">👤 Informasi Pelanggan</h3>
                    ...
                    {/* Payment Info */}
                    <div className="rounded-2xl p-5 border border-gray-200">
                        <h3 className="font-bold text-sm mb-4 text-gray-700">💳 Pembayaran</h3>
                        <div className="space-y-3">
                            <InfoRow label="Metode" value={paymentMethodMap[order.paymentMethod] || order.paymentMethod} />
                            <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-[#f8f9fb]">
                    ...
                    {order.detailOrders && order.detailOrders.length > 0 && (
                        <div className="rounded-2xl p-5 border border-gray-200">
                            <h3 className="font-bold text-sm mb-4 text-gray-700">🛍 Item Pesanan</h3>
                            <div className="space-y-3">
                                {order.detailOrders.map((item, idx) => (
                                    <div
                                        key={idx}
                                        className={`flex items-start justify-between gap-3 py-3 ${
                                            idx < order.detailOrders!.length - 1 ? "border-b border-gray-100" : ""
                                        }`}
                                    >
                                        ...
                                        <p className="text-sm font-bold flex-shrink-0 text-gray-700">
